...
#Bereinigen der Messwerte und Fehlerformel
D=(U_aus**2)-(U_V**2)
...
plt.plot(x, linear(popt[0],x), linestyle='-', label='Fitfunktion $f(x)=c*x$') #Fit plotten
...
plt.text(1000, 17, '$c=$%.2e $\pm$ %.2e' %(popt[0], stdfehler ), fontsize=10)
plt.text(1000, 15.5, '$\chi^{2}=$ %.1f' %(chisquare), fontsize=10)
plt.text(1000, 14, '$\chi_{red}^{2}=$ %.1f ' %(chisquare_red), fontsize=10)
plt.text(1000, 12.5, '$P=$ %.2f ' %(prob), fontsize=10)
